...
Each chapter builds on the previous one in a single linear progression. The Start Tag for a chapter is the End Tag of the prior chapter; Chapter 1 begins from the untagged baseline that precedes `chapter-01-bootstrap`. The End Tag names the commit readers can checkout to review or return to that milestone.

| Chapter | Start Tag (planned) | End Tag (planned) | Verification |
| --- | --- | --- | --- |
| 01 Bootstrap | untagged baseline before `chapter-01-bootstrap` | `chapter-01-bootstrap` | `cd nanocpa && go test ./internal/api -run 'TestServer_'` |
| 02 Config | `chapter-01-bootstrap` | `chapter-02-config` | `cd nanocpa && go test ./internal/config` |
| 03 Access | `chapter-02-config` | `chapter-03-access` | `cd nanocpa && go test ./internal/access && cd nanocpa && go test ./internal/api -run 'Test.*Middleware'` |
...
| 08 Routing and Hardening | `chapter-07-claude-provider` | `chapter-08-routing-and-hardening` | `cd nanocpa && go test ./internal/auth ./internal/api -run 'TestManager_|TestServer_' && cd nanocpa && go test ./...` |

## Tag Conventions

- Chapter tags all use the prefix `chapter-` followed by a two-digit number and a short name (e.g., `chapter-04-openai-surface`).
- Start tags are the prior chapter's end tag; the first chapter begins at the untagged baseline before `chapter-01-bootstrap` because there is no prior tutorial tag.
