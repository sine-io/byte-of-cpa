...
func (h *Handler) refreshGeminiOAuthAccessToken(ctx context.Context, auth *coreauth.Auth) (string, error) {
	...
	conf := &oauth2.Config{
		ClientID:     firstNonEmpty(stringValue(base, "client_id"), stringValue(metadata, "client_id"), geminiAuth.OAuthClientID()),
		ClientSecret: firstNonEmpty(stringValue(base, "client_secret"), stringValue(metadata, "client_secret"), geminiAuth.OAuthClientSecret()),
...
func (h *Handler) refreshAntigravityOAuthAccessToken(ctx context.Context, auth *coreauth.Auth) (string, error) {
	...
	form := url.Values{}
	form.Set("client_id", firstNonEmpty(stringValue(metadata, "client_id"), antigravity.OAuthClientID()))
	form.Set("client_secret", firstNonEmpty(stringValue(metadata, "client_secret"), antigravity.OAuthClientSecret()))
