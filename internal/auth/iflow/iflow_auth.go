...
const (
	iFlowOAuthClientIDEnv     = "IFLOW_OAUTH_CLIENT_ID"
	iFlowOAuthClientSecretEnv = "IFLOW_OAUTH_CLIENT_SECRET"
)
...
func oauthClientID() string {
	return strings.TrimSpace(os.Getenv(iFlowOAuthClientIDEnv))
}

func oauthClientSecret() string {
	return strings.TrimSpace(os.Getenv(iFlowOAuthClientSecretEnv))
}
...
func (ia *IFlowAuth) newTokenRequest(ctx context.Context, form url.Values) (*http.Request, error) {
	...
	basic := base64.StdEncoding.EncodeToString([]byte(oauthClientID() + ":" + oauthClientSecret()))
